...
# copie des fichiers sql dans le dossier daté
	...
	echo "Les scripts de version $VERSION_NAME ont été copiés dans le dossier de déploiement"
				
else
	echo "les scripts de la $VERSION_NAME ne peuvent pas être copiés dans le dossier de déploiement "
	echo "il y a toujours des scripts rejetés dans la version $VERSION_NAME, soit ils sont mal nommés,ou ils ont échoué pendant le test (verifier la table de register)"
	exit 1
fi

																										

echo "Creation du dossier sql dans CIFS"
# TARGET_SQL_FOLDER=$TARGET_BASE_FOLDER/$PLATEFORME/$VERSION_NUMBER/$CURRENT_INCREMENT/sql
# ssh -i /appli/.ssh/id_rsa -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@${TARGET_HOST} bash -c "'mkdir -p ${TARGET_SQL_FOLDER}'"  |& tee -a ${echo_FILE}
# if [ ${PIPESTATUS[0]} -ne "0" ]; then
# 	echo "Erreur lors de la creation du dossier cible dans CIFS"
# 	echo "FIN DES TRAITEMENTS AVEC ERREUR VOIR FICHIER DE echo ${echo_FILE}"
# 	exit 1
# fi

# if [ $SCRIPT_HANDLED_TO_BE_DEPLOYED_LIST_COUNT -gt "0" ]; then
# 	echo "copie des dossiers contenant les fichiers sql dans les dossiers cibles sur cifs"
# 	scp -i /appli/.ssh/id_rsa -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -r ${VERSIONED_SQL_DEPLOYMENT_DIRECTORY_PROCESSED}/$DATE_NOW/* root@${TARGET_HOST}:${TARGET_SQL_FOLDER}  |& tee -a ${echo_FILE}
# 	if [ ${PIPESTATUS[0]} -ne "0" ]; then
# 		echo "Erreur lors de la copie des scripts sql dans le dossier cible sous CIFS"
# 		echo "FIN DES TRAITEMENTS AVEC ERREUR VOIR FICHIER DE echo ${echo_FILE}"
# 		exit 1
# 	fi
# fi
...
# # rm -rf ${VERSIONED_SQL_DEPLOYMENT_DIRECTORY_PROCESSED}/$DATE_NOW |& tee -a ${echo_FILE}
# rm -rf appli/deployment/sql/$PLATEFORME/$VERSION_NUMBER/PROCESSED/$DATE_NOW |& tee -a ${echo_FILE}
...
echo "${TIMESTAMP}|BUILD_RUNNING" >> appli/deployment/sql/$PLATEFORME/$VERSION_NUMBER/register
	if [ $count -gt "0" ]
		...
		echo " fin de traitement avec succes "
		for (( c=0; c<$count; c++ ))
		do
