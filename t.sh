...
. C:/Program Files (x86)/Jenkins/workspace/INT1_JOB/environment_config.sh 
		...
		then
			...
			if [[ ! ($INODE_UPPERCASE =~ $SCRIPT_NAME_REGEXP) ]] || [[ $INODE_UPPERCASE != *"${DB_NAME_IN_SCRIPT_UPPERCASE}"* ]]
			...
			else
				{
					...
					# echo "************"+"$1|$inode|${CHECKSUM_VALUE}|${COPY_TARGET}|${COPY_TARGET2}|${SCRIPT_PF_DEPENDANT}|${NAME_PF_PART}"
...
list_script_alredy_succes=( $( mysql --batch mysql -u $username -p$password -N -e "use db5; select script_name from scripts where  script_handled='encour' and script_name_regex ='ok' or  script_handled='encour' and script_name_regex ='ok' and script_state ='invalid'"  ) )
